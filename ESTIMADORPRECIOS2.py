...
if 'results' not in st.session_state:
    st.session_state.results = pd.DataFrame(columns=['Model', 'R²', 'RMSE', 'Training Time'])
...
if st.session_state.models_trained:
    ...
    with st.expander(text["linear_regression"]):
        if st.button(text["show_linear"]):
            ...
            try:
                model_results = st.session_state.results.loc[[text["linear_regression"]]]
                st.dataframe(model_results[['R²', 'RMSE', 'Training Time']]
                            .style.format({'R²': '{:.3f}', 'RMSE': '{:.3f}', 'Training Time': '{:.2f}'}))
            except Exception as e:
    ...
    with st.expander(text["mlp"]):
        if st.button(text["show_mlp"]):
            try:
                model_results = st.session_state.results.loc[[text["mlp"]]]
                st.dataframe(model_results[['R²', 'RMSE', 'Training Time']]
                            .style.format({'R²': '{:.3f}', 'RMSE': '{:.3f}', 'Training Time': '{:.2f}'}))
                
                if text["mlp"] in st.session_state.get('loss_curves', {}):
                    fig = go.Figure()
                ...
                st.error(f"Error al mostrar resultados MLP: {str(e)}")

    # Red Neuronal Regularizada
    with st.expander(text["nn_with_reg"]):
        if st.button(text["show_nn"]):
            try:
                model_results = st.session_state.results.loc[[text["nn_with_reg"]]]
                st.dataframe(model_results[['R²', 'RMSE', 'Training Time']]
                            .style.format({'R²': '{:.3f}', 'RMSE': '{:.3f}', 'Training Time': '{:.2f}'}))
                
                if text["nn_with_reg"] in st.session_state.get('loss_curves', {}):
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(
                ...
                st.error(f"Error al mostrar resultados RN: {str(e)}")
