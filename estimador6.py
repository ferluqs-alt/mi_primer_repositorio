...
def display_eda_results(eda_results, target_var):
    ...
    col1.metric(t('num_rows'), eda_results['dimensions']['num_rows'])
    col2.metric(t('num_cols'), eda_results['dimensions']['num_cols'])
    ...
    st.metric("Duplicated Rows", eda_results['duplicates'])
    ...
    if eda_results['numeric_cols']:
        ...
        for i, col in enumerate(eda_results['numeric_cols'][:num_cols_to_show]):
            axes[i].hist(st.session_state.df[col].dropna(), bins=30, alpha=0.7, color='blue')
    ...
    if eda_results['numeric_cols_list']:
        ...
        for i, col in enumerate(eda_results['numeric_cols_list'][:num_cols_to_show]):
            axes[i].boxplot(st.session_state.df[col].dropna())
            ...
            if numeric_cols:
                ...
                if st.button(t('run_eda')):
                    with st.spinner('Performing EDA...'):
                        st.session_state.eda_results = perform_eda(df, st.session_state.target_var)
