...
translations = {
    ...
    "fr": {
        ...
        "show_null_summary": "Afficher le résumé des valores manquantes",
...
def translate_dataframe(df, language):
    ...
    # Remove any duplicate columns that might have been created during translation
    # This prevents issues like having both "ocean_proximity" and "proximidad_océano"
    ...
    if st.checkbox(get_text("show_null_summary"), value=True):
        ...
        if len(null_summary) > 0:
            ...
            st.dataframe(pd.DataFrame({
                ...
                'Porcentaje': (null_summary.values / len(df) * 100).round(2)
            }))
...
def perform_eda(df, target_var):
    ...
    try:
        ...
        missing_df = pd.DataFrame({
            ...
            'Porcentaje': (missing_data.values / len(df)) * 100
        })
        ...
        st.error(f"Error realizando EDA: {str(e)}")
...
def display_eda_results(eda_results, target_var):
    ...
    if not eda_results:
        st.error("No hay resultados de EDA disponibles")
        return
    ...
    if normality_tests:
        ...
        normality_df['Requiere Normalización'] = ~normality_df['Normal']
...
def main():
    ...
    if st.session_state.df is not None:
        ...
        if st.session_state.evaluation_results and st.session_state.show_evaluation:
            ...
            ax.barh(
                best_model_importance['Característica'].head(10),
                best_model_importance['Importancia'].head(10)
            )
