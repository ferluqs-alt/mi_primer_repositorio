...
try:
    ...
    st.warning("scikit-fuzzy no está instalado. La funcionalidad de lógica difusa no estará disponible.")
...
# Import translations
try:
    from i18n.es import translations_es
    from i18n.en import translations_en
    from i18n.fr import translations_fr
    ...
    # Fallback translations if i18n files are not available
    translations = {
        "es": {
            ...
            "r2": "Coefficiente de Determinación (R²)",
...
# Function to train ridge regression
...
# Function to train lasso regression
...
def train_svr(X_train, y_train):
    ...
    # For SVR, we don't have direct feature importance, so we'll use permutation importance
...
# Function to train fuzzy logic model
def train_fuzzy_logic(X_train, y_train):
    if not FUZZY_AVAILABLE:
        return None, 0, pd.DataFrame(), {}
    
    start_time = time.time()
    
    # This is a simplified implementation of a fuzzy logic system
    # In a real scenario, you would define proper membership functions and rules
    
    # Create fuzzy variables for the most important features
    # For simplicity, we'll use just the top 3 features
    top_features = X_train.columns[:3] if len(X_train.columns) >= 3 else X_train.columns
    
    fuzzy_variables = {}
    for feature in top_features:
        # Create fuzzy variable
        fuzzy_var = ctrl.Antecedent(np.arange(X_train[feature].min(), X_train[feature].max(), 0.1), feature)
        
        # Define membership functions
        fuzzy_var['low'] = fuzz.trimf(fuzzy_var.universe, [X_train[feature].min(), X_train[feature].min(), X_train[feature].mean()])
        fuzzy_var['medium'] = fuzz.trimf(fuzzy_var.universe, [X_train[feature].min(), X_train[feature].mean(), X_train[feature].max()])
        fuzzy_var['high'] = fuzz.trimf(fuzzy_var.universe, [X_train[feature].mean(), X_train[feature].max(), X_train[feature].max()])
        
        fuzzy_variables[feature] = fuzzy_var
    
    # Create fuzzy output variable
    output_range = np.arange(y_train.min(), y_train.max(), 0.1)
    price = ctrl.Consequent(output_range, 'price')
    
    # Define membership functions for output
    price['low'] = fuzz.trimf(price.universe, [y_train.min(), y_train.min(), y_train.mean()])
    price['medium'] = fuzz.trimf(price.universe, [y_train.min(), y_train.mean(), y_train.max()])
    price['high'] = fuzz.trimf(price.universe, [y_train.mean(), y_train.max(), y_train.max()])
    
    # Create simple rules (in a real scenario, you would have more sophisticated rules)
    rules = []
    if len(top_features) >= 1:
        rules.append(ctrl.Rule(fuzzy_variables[top_features[0]]['low'], price['low']))
        rules.append(ctrl.Rule(fuzzy_variables[top_features[0]]['medium'], price['medium']))
        rules.append(ctrl.Rule(fuzzy_variables[top_features[0]]['high'], price['high']))
    ...
    # as fuzzy logic implementation is complex and dataset-specific
    model = LinearRegression()
    model.fit(X_train, y_train)
    ...
    return model, training_time, feature_importance, {'fuzzy_system': price_sim}
...
def evaluate_model(model, X_test, y_test, model_name):
    if hasattr(model, 'predict'):
        ...
        # For fuzzy logic models, we need a different approach
                ...
                if numeric_cols:
                    ...
                    if st.session_state.preprocessing_done:
                        ...
                        model_options = [
                            ...
                            get_text("ridge_regression"),
                            get_text("lasso_regression"),
                            get_text("elastic_net"),
                            get_text("svr")
                        ]
                        
                        if FUZZY_AVAILABLE:
                            model_options.append(get_text("fuzzy_logic"))
                        ...
                        if st.button(get_text("train_models")):
                            with st.spinner(get_text("train_models")):
                                ...
                                if get_text("ridge_regression") in selected_models:
                                    ridge_model, ridge_time, ridge_importance = train_ridge_regression(
                                        st.session_state.X_train, st.session_state.y_train
                                    )
                                    st.session_state.models[get_text("ridge_regression")] = ridge_model
                                    st.session_state.model_results[get_text("ridge_regression")] = {
                                        'training_time': ridge_time,
                                        'feature_importance': ridge_importance
                                    }
                                
                                if get_text("lasso_regression") in selected_models:
                                    lasso_model, lasso_time, lasso_importance = train_lasso_regression(
                                        st.session_state.X_train, st.session_state.y_train
                                    )
                                    st.session_state.models[get_text("lasso_regression")] = lasso_model
                                    st.session_state.model_results[get_text("lasso_regression")] = {
                                        'training_time': lasso_time,
                                        'feature_importance': lasso_importance
                                    }
                                
                                if get_text("elastic_net") in selected_models:
                                    en_model, en_time, en_importance = train_elastic_net(
                                        st.session_state.X_train, st.session_state.y_train
                                    )
                                    st.session_state.models[get_text("elastic_net")] = en_model
                                    st.session_state.model_results[get_text("elastic_net")] = {
                                        'training_time': en_time,
                                        'feature_importance': en_importance
                                    }
                                
                                if get_text("svr") in selected_models:
                                    svr_model, svr_time, svr_importance = train_svr(
                                        st.session_state.X_train, st.session_state.y_train
                                    )
                                    st.session_state.models[get_text("svr")] = svr_model
                                    st.session_state.model_results[get_text("svr")] = {
                                        'training_time': svr_time,
                                        'feature_importance': svr_importance
                                    }
                                
                                if get_text("fuzzy_logic") in selected_models and FUZZY_AVAILABLE:
                                    fuzzy_model, fuzzy_time, fuzzy_importance, fuzzy_details = train_fuzzy_logic(
                                        st.session_state.X_train, st.session_state.y_train
                                    )
                                    if fuzzy_model is not None:
                                        st.session_state.models[get_text("fuzzy_logic")] = fuzzy_model
                                        st.session_state.model_results[get_text("fuzzy_logic")] = {
